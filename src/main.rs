...
fn clean_cache() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for entry in std::fs::read_dir(&cache_dir)? {
        ...
        if path.is_dir() {
            println!("  Deleting: {}", path.file_name().unwrap().to_string_lossy());
            count += 1;
        }
    }
...
struct Args {
    ...
    #[arg(long, default_value = "vits-piper-es_AR-daniela-high", value_name = "MODEL")]
    model: String,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(model_name) = &args.delete_model { return delete_model(model_name); }
    if let Some(model_name) = &args.model_info { return show_model_info(model_name); }
    if args.update_models { return update_models(); }
    if args.clean_cache { return clean_cache(); }
    ...
    if args.list_models {
        ...
        for model in MODELS {
            let description = match model.name {
                "vits-piper-es_AR-daniela-high" => "Argentine Spanish, female, high quality (DEFAULT)",
            ...
            let marker = if model.name == "vits-piper-es_AR-daniela-high" { " *" } else { "" };
    ...
    let (mut rate, mut pitch, mut volume, mut noise_scale_w, mut length_scale, mut silence_scale, mut sid) =
    ...
    if let Some(r) = args.rate { rate = r; }
    if let Some(p) = args.pitch { pitch = p; }
    if let Some(v) = args.volume { volume = v; }
    if let Some(nw) = args.noise_scale_w { noise_scale_w = nw; }
    if let Some(ls) = args.length_scale { length_scale = ls; }
    if let Some(ss) = args.silence_scale { silence_scale = ss; }
    if let Some(s) = args.sid { sid = Some(s); }
    ...
    let config = OfflineTtsConfig {
        model: OfflineTtsModelConfig { vits: vits_config, ..Default::default() },
        rule_fsts: None,
        max_num_sentences: 1,
        rule_fars: None,
        silence_scale,
    ...
    let gen_config = sherpa_onnx::GenerationConfig { speed: rate, sid: sid.unwrap_or(0), ..Default::default() };
    let callback: Option<fn(&[f32], f32) -> bool> = None;
    let audio = tts.generate_with_config(&text, &gen_config, callback).expect("Failed to generate audio");

    // Process samples
    let mut samples: Vec<f32> = audio.samples().iter().map(|&x| (x * volume).clamp(-1.0, 1.0)).collect();
    ...
    let samples = if output_sample_rate != sample_rate {
        tracing::debug!("Resampling from {} to {} Hz...", sample_rate, output_sample_rate);
        resample(&samples, sample_rate, output_sample_rate)?
    } else { samples };

    // Determine output format
    let output_format = if args.format == "auto" {
        if args.stdout { "mp3".to_string() }
        else {
            match std::path::Path::new(&args.output).extension().and_then(|s| s.to_str()).unwrap_or("mp3").to_lowercase().as_str() {
                "wav" => "wav".to_string(),
                _ => "mp3".to_string(),
            }
        }
...
fn apply_preset(preset_name: &str) -> PresetResult {
    match preset_name.to_lowercase().as_str() {
        ...
        _ => Err(Box::new(std::io::Error::new(
            ...
            format!("Unknown preset: '{}'. Available: human, fast, slow, quiet, loud", preset_name)
        )))
    }
}
...
fn normalize_audio(samples: &[f32]) -> Vec<f32> {
    if samples.is_empty() { return samples.to_vec(); }
    let max_amplitude = samples.iter().map(|&x| x.abs()).fold(0.0f32, |a, b| a.max(b));
    if max_amplitude < 0.001 { return samples.to_vec(); }
    let gain = 0.95 / max_amplitude;
    samples.iter().map(|&x| x * gain).collect()
}

fn resample(samples: &[f32], input_rate: u32, output_rate: u32) -> Result<Vec<f32>, Box<dyn std::error::Error>> {
    if input_rate == output_rate { return Ok(samples.to_vec()); }
...
fn write_wav(samples: &[f32], sample_rate: u32, args: &Args) -> Result<(), Box<dyn std::error::Error>> {
    let spec = hound::WavSpec { channels: 1, sample_rate, bits_per_sample: 16, sample_format: hound::SampleFormat::Int };
    if args.stdout {
        let mut cursor = std::io::Cursor::new(Vec::new());
        let mut writer = hound::WavWriter::new(&mut cursor, spec)?;
        for &sample in samples { writer.write_sample((sample.clamp(-1.0, 1.0) * 32767.0) as i16)?; }
        writer.finalize()?;
...
fn write_mp3(samples: &[f32], sample_rate: u32, args: &Args) -> Result<(), Box<dyn std::error::Error>> {
    let pcm: Vec<i16> = samples.iter().map(|&x| (x.clamp(-1.0, 1.0) * 32767.0) as i16).collect();
    let bitrate = match args.bitrate { 64 | 96 | 128 | 192 | 256 | 320 => args.bitrate, _ => { eprintln!("Warning: Unsupported bitrate {}, using 128 kbps", args.bitrate); 128 } };
    let config = Mp3EncoderConfig::new().sample_rate(sample_rate).bitrate(bitrate).channels(1).stereo_mode(StereoMode::Mono);
    let mut encoder = Mp3Encoder::new(config)?;
    let mut mp3 = Vec::new();
    for frame in encoder.encode_interleaved(&pcm)? { mp3.extend_from_slice(&frame); }
    mp3.extend_from_slice(&encoder.finish()?);
    if args.stdout { io::stdout().write_all(&mp3)?; println!("Success! Audio written to stdout"); }
    else { File::create(&args.output)?.write_all(&mp3)?; println!("Success! Saved speech to '{}'", args.output); }
    Ok(())
}
